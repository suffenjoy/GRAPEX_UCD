...
ggplot(PRI_5day, aes(x = Date, y = PRI, color = as.factor(Site))) + geom_point() + geom_line() + ggtitle("PRI Time Series (5-Day-Average)") + labs(color = "Site")
...
cimis_noon <- aggregate(cimis_noon[,c("Air_Temp","Rel_Hum")], by = list("Date"  = cimis_noon$Date), FUN = mean)
...
ggplot(PRIdaily_s2, aes(x = Date, y= PRI_LAI_scale, color = Site)) + geom_point() + geom_line()
...
```
#####Block 2 on 2019-08-22
```{r}
ggplot(subset(noon_selection(PRIma_weather, 10, 18), Date == "2019-08-20" & Site == "Block1"), aes(x = Time)) + geom_line(aes(y = PRI.4.ma, color = "PRI"), size = 0.8)
